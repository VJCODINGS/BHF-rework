                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('roles.update', $role->id) }}" method="post">
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-4 justify-center sm:justify-start">
                                @if ($permissions->isNotEmpty())
                                    ...
                                    <div class="mt-3 w-full sm:w-auto">
                                        <input {{ ($hasPermissions->contains($permission->name)) ? 'checked ' : ''}} id="permission-{{ $permission->id }}" type="checkbox" class="rounded" name="permission[]" value="{{ $permission->name }}">
                            ...
                            <button class="bg-slate-700 text-sm rounded-md px-5 py-3 text-white mt-4">Update</button>
                        </div>
                    </form>
                </div>
            </div>
